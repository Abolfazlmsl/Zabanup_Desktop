...
import QtQuick.Controls 1.4 as QQC1_4
    ...
    Rectangle{
        ...
        Rectangle{
                ...
                ListView{
                    ...
                    delegate: FiltersButton{
                        ...
                        onClickSubjectFilter: {
                            ...
                            if(model_Filter.get(index).name === "انتخاب تاریخ"){
                                date_Popup.open()
                            }
            ...
            Rectangle{
                ...
                //-- for get count of "Date" Type in Filter Model --//
                property real testFilter_Count: {
                    var counter = 0;
                    for(var i=0; i<model_Filter.count; i++){
                        if(model_Filter.get(i).type === "TestType"){
                            counter++;
                        }
                    }
                    log("Count Test Filter on Model : " + counter)
                    return counter;
                }

                //-- for get count of None "Date" Type in Filter Model --//
                property real noneTestFilter_Count: {
                    var counter = 0;
                    for(var i=0; i<model_Filter.count; i++){
                        if(model_Filter.get(i).type !== "TestType"){
                            counter++;
                    ...
                    log("Count None Test Filter on Model : " + counter)
                    return counter;
                }
                ...
                width: parent.width - (10 * ratio)
                height: (body_FilterTest.testFilter_Count * (35 * ratio)) +
                        ((body_FilterTest.testFilter_Count - 1) * lst_FilterTest.spacing) +
                        (lst_FilterTest.topMargin + lst_FilterTest.bottomMargin)/* +
                        (body_FilterTest.noneTestFilter_Count * lst_FilterTest.spacing)*/
                anchors.top: body_FilterDate.bottom
                anchors.topMargin: 5 * ratio
                anchors.horizontalCenter: parent.horizontalCenter
                ...
                ListView{
                    ...
                    ScrollBar.vertical: ScrollBar {
                        width: 10 * ratio
                        policy: ScrollBar.AsNeeded
                        parent: lst_FilterTest.parent
                        anchors.top: lst_FilterTest.top
                        anchors.left: lst_FilterTest.left
                        anchors.bottom: lst_FilterTest.bottom
                    }
                    ...
                    delegate: FiltersButton{
                        ...
                        width: parent.width - (16 * ratio) // (LeftMargin + rightMargin) - Left and Right Border
                        height: (model.type === "TestType") ? 35 * ratio : -4 * ratio //  (-4 * ratio) is for remove spacing for noneTestFilter_Count
                        visible: (model.type === "TestType") ? true : false
                        anchors.horizontalCenter: parent.horizontalCenter
                        subjText: model.name
...
//                        name: "LineSeries"
    ...
    Popup{
        ...
        x: body_FilterDate.x + body_FilterDate.height
        y: body_FilterDate.y + body_FilterDate.width

        QQC1_4.Calendar{
            minimumDate: new Date(2017, 0, 1)
            maximumDate: new Date(2021, 0, 1)
            locale: Qt.locale("fa-IR")
        }
    }
